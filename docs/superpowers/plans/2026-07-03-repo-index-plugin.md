# repo-index Plugin Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A publicly installable Claude Code plugin whose `repo-index` skill scans a folder for local git repos and generates a self-contained interactive HTML index.

**Architecture:** This repo is both the plugin and its own marketplace (`.claude-plugin/marketplace.json` at root, plugin under `plugins/repo-index/`). One Python-stdlib script (`scan_repos.py`) does everything: discover repos, collect metadata via local git commands, and emit a single HTML file with inline CSS/JS and the data embedded as JSON. A plain-assert test script builds synthetic git repos in a temp dir and verifies every collected field plus the CLI.

**Tech Stack:** Python 3.9+ stdlib only; git CLI; vanilla HTML/CSS/JS (zero external resources).

## Global Constraints

- Python 3.9+ **standard library only** — no pip dependencies, ever.
- **No network access** during scan: no GitHub API, no `git fetch`. All git reads are local.
- Output is **one self-contained HTML file**: no CDN scripts, stylesheets, fonts, or remote images.
- Plugin name: `repo-index`. Marketplace name: `claude-repo-index`. Version: `0.1.0`. GitHub: `lopperman/claude-repo-index`.
- Tests are plain-assert, run with `python3 tests/test_scan_repos.py` (no pytest). Fixtures need git ≥ 2.28 (`git init -b main`).
- A failure scanning any single repo must never abort the scan (log to stderr, skip).
- Spec: `docs/superpowers/specs/2026-07-03-repo-index-plugin-design.md`.

## File Structure

```
.claude-plugin/marketplace.json                          # Task 1 — marketplace catalog
plugins/repo-index/.claude-plugin/plugin.json            # Task 1 — plugin manifest
plugins/repo-index/skills/repo-index/scripts/scan_repos.py  # Tasks 2-4 — scanner + HTML generator
tests/test_scan_repos.py                                 # Tasks 2-4 — fixture + all tests
plugins/repo-index/skills/repo-index/SKILL.md            # Task 5 — skill instructions
README.md                                                # Task 5 — install/usage docs (modify)
```
...
- Create: `plugins/repo-index/.claude-plugin/plugin.json`

**Interfaces:**
- Consumes: nothing.
- Produces: marketplace name `claude-repo-index` and plugin name `repo-index` used verbatim in Task 5's install docs.

- [ ] **Step 1: Create `.claude-plugin/marketplace.json`**

```json
{
  "name": "claude-repo-index",
  "owner": {
    "name": "Paul Brower"
  },
  "plugins": [
    {
      "name": "repo-index",
      "source": "./plugins/repo-index",
      "description": "Scan a folder for local git repos and generate a searchable, self-contained HTML index page."
    }
  ]
}
```

- [ ] **Step 2: Create `plugins/repo-index/.claude-plugin/plugin.json`**

```json
{
  "name": "repo-index",
  "description": "Scan a folder recursively for local git repositories and generate a self-contained interactive HTML index — search, sort, status badges, activity sparklines, and expandable READMEs.",
  "version": "0.1.0"
}
```

- [ ] **Step 3: Verify both manifests parse and have required fields**
...
assert m['name'] == 'claude-repo-index' and m['owner']['name'] and m['plugins'][0]['source'] == './plugins/repo-index'
p = json.load(open('plugins/repo-index/.claude-plugin/plugin.json'))
assert p['name'] == 'repo-index' and p['description'] and p['version'] == '0.1.0'
print('manifests ok')
"
```
...
```bash
git add .claude-plugin plugins/repo-index/.claude-plugin
git commit -m "feat: add plugin and marketplace manifests"
```

---

### Task 2: Repo discovery and remote-URL normalization

**Files:**
- Create: `plugins/repo-index/skills/repo-index/scripts/scan_repos.py`
- Create: `tests/test_scan_repos.py`

**Interfaces:**
- Consumes: nothing.
- Produces (in `scan_repos.py`, used by Tasks 3-4):
  - `SKIP_DIRS: set[str]`
  - `run_git(repo, *args) -> str | None` — stdout stripped on success, `None` on any failure.
  - `find_repos(root: Path) -> list[Path]` — dirs containing `.git` (file or dir), no descent into found repos, skips `SKIP_DIRS`, hidden dirs, and symlinked dirs.
  - `normalize_remote(url: str | None) -> str | None` — ssh/scp forms → `https://host/path` (no `.git`); plain http(s) kept minus `.git`; anything else (e.g. filesystem paths) → `None`.
- Produces (in `tests/test_scan_repos.py`, used by Tasks 3-4): module-level `E` (git env), `git(cwd, *args)`, `make_repo(d: Path) -> Path`, `build_fixture(tmp: Path) -> Path` (returns the scan root containing repos `work/alpha`, `work/bravo`, `grp/charlie`, a decoy repo under `work/node_modules/`, and a plain `notarepo/` dir).
...
Run: `python3 tests/test_scan_repos.py`
Expected: FAIL with `ModuleNotFoundError: No module named 'scan_repos'`
...
```bash
git add plugins/repo-index/skills/repo-index/scripts/scan_repos.py tests/test_scan_repos.py
git commit -m "feat: repo discovery and remote URL normalization"
```

---

### Task 3: Per-repo metadata collection

**Files:**
- Modify: `plugins/repo-index/skills/repo-index/scripts/scan_repos.py` (append below `normalize_remote`)
- Modify: `tests/test_scan_repos.py`

**Interfaces:**
- Consumes: `run_git`, `normalize_remote` from Task 2.
- Produces: `collect_repo(repo: Path, root: Path) -> dict` with exactly these keys — `name` (str), `path` (str, absolute), `rel_path` (str), `parent` (str, `"."` for top level), `remote_url` (str|None), `branch` (str), `dirty` (bool), `stashes` (int), `ahead` (int), `behind` (int), `has_upstream` (bool), `last_commit` (`{"date","message","author"}`|None), `modified_ts` (float, unix seconds), `language` (str|None), `lang_color` (str hex), `size_bytes` (int), `commit_count` (int), `sparkline` (list of 12 ints, oldest month first), `description` (str), `readme` (str). Task 4's HTML JS reads these exact key names.

- [ ] **Step 1: Add failing tests**

In `tests/test_scan_repos.py`, add after `test_find_repos`:
...
And in `main()`, after `test_find_repos(fx)` add:
...
Run: `python3 tests/test_scan_repos.py`
Expected: FAIL with `AttributeError: module 'scan_repos' has no attribute 'collect_repo'`

- [ ] **Step 3: Write the implementation**

Append to `scan_repos.py`:
...
```bash
git add plugins/repo-index/skills/repo-index/scripts/scan_repos.py tests/test_scan_repos.py
git commit -m "feat: collect per-repo git metadata, language, readme, sparkline"
```

---

### Task 4: HTML generation and CLI

**Files:**
- Modify: `plugins/repo-index/skills/repo-index/scripts/scan_repos.py` (append below `collect_repo`)
- Modify: `tests/test_scan_repos.py`

**Interfaces:**
- Consumes: `find_repos`, `collect_repo` (Tasks 2-3); the repo-dict key names from Task 3 are referenced by name inside the JS.
- Produces: `build_html(repos: list[dict], root: Path) -> str`; `main(argv=None) -> int` (exit 0 ok, 2 bad input); CLI writes `<folder>/repo-index.html` (or `-o PATH`) and prints `N repos -> <path>`.

- [ ] **Step 1: Add failing tests**

In `tests/test_scan_repos.py`, add after `test_charlie_content`:
...
And in `main()`, after `test_charlie_content(fx)` add:
...
Run: `python3 tests/test_scan_repos.py`
Expected: FAIL — with no `main()`/`__main__` block yet, the subprocess exits 0 producing no output, so `test_cli_writes_html` fails its `"3 repos" in r.stdout` assertion.

- [ ] **Step 3: Write the implementation**

Append to `scan_repos.py` (the template is one raw string; `__DATA__` must be substituted **last** so README content can't collide with the other placeholders):
...
- [ ] **Step 5: Eyeball the page once**

Run:
```bash
python3 plugins/repo-index/skills/repo-index/scripts/scan_repos.py ~/projects/personal -o /tmp/repo-index-preview.html
open /tmp/repo-index-preview.html
```
Expected: page renders with tiles, cards, badges, sparklines; search/sort/chips/README expand all work. Fix any JS console errors before committing.

- [ ] **Step 6: Commit**

```bash
git add plugins/repo-index/skills/repo-index/scripts/scan_repos.py tests/test_scan_repos.py
git commit -m "feat: self-contained HTML index generation and CLI"
```

---

### Task 5: SKILL.md and README

**Files:**
- Create: `plugins/repo-index/skills/repo-index/SKILL.md`
- Modify: `README.md` (replace contents)

**Interfaces:**
- Consumes: CLI contract from Task 4 (`scan_repos.py <folder>` writes `<folder>/repo-index.html`); names from Task 1 (`repo-index@claude-repo-index`).
- Produces: user-facing docs; nothing programmatic.

- [ ] **Step 1: Create `plugins/repo-index/skills/repo-index/SKILL.md`**
...
python3 -c "
import re
t = open('plugins/repo-index/skills/repo-index/SKILL.md').read()
m = re.match(r'^---\n(.*?)\n---\n', t, re.S)
assert m and 'name: repo-index' in m.group(1) and 'description:' in m.group(1)
...
```bash
git add plugins/repo-index/skills/repo-index/SKILL.md README.md
git commit -m "docs: skill instructions and install/usage README"
```
...
- Produces: confirmation the plugin installs and the page works on real data.

- [ ] **Step 1: Full test suite from a clean tree**

Run: `git status --porcelain` (expect empty) then `python3 tests/test_scan_repos.py`
Expected: `all tests passed`

- [ ] **Step 2: Real-data run**

Run:
```bash
python3 plugins/repo-index/skills/repo-index/scripts/scan_repos.py ~/projects
open ~/projects/repo-index.html
```
Expected: `N repos -> ...` with N matching reality; page renders; search `claude*`, toggle "Needs attention", switch to parent-directory sort, expand a README. Any stderr `skip` lines investigated.

- [ ] **Step 3: Local install check (user-assisted)**

In a Claude Code session, the user runs:
```
/plugin marketplace add /Users/browerp/projects/personal/claude-skill-repo-index
/plugin install repo-index@claude-repo-index
```
Expected: plugin installs; `/repo-index <folder>` triggers the skill. This validates the manifests end-to-end before publishing.

- [ ] **Step 4: Push (only with user's explicit go-ahead)**

```bash
git push
```
Then the public install path works: `/plugin marketplace add lopperman/claude-repo-index`.
